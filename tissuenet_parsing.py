...
def save_image(numpy_array, image_name, image_type):
    print(numpy_array.shape, np.min(numpy_array), np.max(numpy_array))
    numpy_min = np.min(numpy_array)
    numpy_max = np.max(numpy_array)
    if image_type == "intensity":
        numpy_array = numpy_array * (65535/(numpy_max - numpy_min))
    img = Image.fromarray(numpy_array.astype(np.uint16))
    print("\t", numpy_array.shape, np.min(numpy_array), np.max(numpy_array))
...
def save_file(npz_location, classofdata, typeofdata, npz_dir, output_dir, suffix):
    ...
    for ex in range(num_examples):
        ...
        save_image(numpy_array=image_array, \
            image_name=image_file, image_type="intensity")
        save_image(numpy_array=groundtruth_array, \
            image_name=groundtruth_file, image_type="label")
        save_image(numpy_array=groundtruth_multiclass_array, \
            image_name=groundtruth_multiclass_file, image_type="label")
        save_image(numpy_array=groundtruth_borderbinary_array, \
            image_name=groundtruth_borderbinary_file, image_type="label")
        save_image(numpy_array=gt_cntrbnry_3pxlerro_arr, \
            image_name=groundtruth_centerbinary3_file, image_type="label")
        save_image(numpy_array=gt_cntrbnry_2pxlerro_arr, \
            image_name=groundtruth_centerbinary2_file, image_type="label")
        # Save the images
